...
def run(settings):
    ...
    data_processing_train = processing.PromptLTRBDenseRegressionProcessing(
        ...
        center_sampling_radius=settings.center_sampling_radius, prompt_mode=prompt_mode)

    data_processing_val = processing.PromptLTRBDenseRegressionProcessing(
        search_area_factor=settings.search_area_factor,
        output_sz=settings.output_sz,
        center_jitter_factor=settings.center_jitter_factor,
        scale_jitter_factor=settings.scale_jitter_factor,
    ...
    net = tompnet.prompt_tompnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True,
                                   head_feat_blocks=0,
                                   ...
                                   mlp_dim=mlp_dim, prompt_mode=prompt_mode)
    ...
    actor = actors.PromptToMPActor(net=net, objective=objective, loss_weight=loss_weight)
