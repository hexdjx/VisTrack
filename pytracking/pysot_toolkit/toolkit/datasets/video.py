...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        # NFS
        # self.img_names = [os.path.join(root, 'sequences', x.split('/')[-2], x.split('/')[-1]) for x in img_names]

        # UAV
        # self.img_names = [os.path.join(root, 'data_seq/UAV123', x.split('/')[-2], x.split('/')[-1]) for x in img_names]

        # LaSOT
        self.img_names = [os.path.join(root, 'Test', x) for x in img_names]
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            ...
            # nfs
            # traj_file = os.path.join(path, name, 'nfs_' + self.name + '.txt')
            # uav
            # traj_file = os.path.join(path, name, 'uav_' + self.name + '.txt')
