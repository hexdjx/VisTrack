...
from pytracking.pysot_toolkit.toolkit.datasets import DatasetFactory
from pytracking.pysot_toolkit.toolkit.utils.region import vot_overlap, vot_float2str
...
parser = argparse.ArgumentParser(description='Pytracking tracking')
...
def main():
    ...
    # ---------------------------------------------------------------
    RF_module = RefineModule(os.path.join(env_settings().network_path, 'SEcmnet_ep0040-c.pth.tar'), selector=0)
    # ---------------------------------------------------------------

    # create dataset
    dataset = DatasetFactory.create_dataset(name=args.dataset,
                                            dataset_root=env_settings().vot18_path,
                                            load_img=False)
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            # save results
            tracker_name = args.tracker_name + '_' + args.tracker_param
            video_path = os.path.join(env_settings().vot18_results_path, tracker_name, 'baseline', video.name)
        ...
        print("{:s} total lost: {:d}".format(os.path.join(args.tracker_name, args.tracker_param), total_lost))
