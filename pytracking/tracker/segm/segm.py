...
from pytracking.utils.bbox_fit import fit_bbox_to_mask
from pytracking.utils.mask_utils import show_mask
...
class Segm(BaseTracker):
    ...
    def initialize(self, image, info: dict, init_mask=None, *args, **kwargs):
    ...
    def track(self, image, info: dict = None):
        ...
        if self.params.use_segmentation:
            if pred_segm_region is not None:
                pred_segm_region = {'target_bbox': pred_segm_region}
    ...
    def extract_processed_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (
        ...
        x, _ = self.extract_sample(im, pos, scales, sz)
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        init_patch_crop, f_ = prutils.sample_target_seg(image, np.array(bb), self.params.segm_search_area_factor,
        ...
        init_mask_patch_np, patch_factor_init = prutils.sample_target_seg(mask, np.array(bb),
        ...
        # conv1 layer1 layer2 for segment, layer3 for
        ...
        if init_mask is None:
            ...
            while iters < 1:
                ...
                if self.params.show_mask:
                    ...
                    show_mask(None, mask, segm_crop_sz, bb, image.shape[1], image.shape[0])
    ...
    def segment_target(self, image, pos, sz):
        ...
        if self.segmentation_task:
            ...
            if self.params.show_mask:
                show_mask(None, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0])
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  # 1000:
            ...
            if self.params.segm_optimize_polygon:
                if not self.segmentation_task:
                    ...
                    # Note: move this below if evaluating on VOT
                    if self.params.show_mask:
                        show_mask(mask, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0])
    ...
    def poly_to_aabbox(self, x_, y_):  # axis-aligned bounding box
