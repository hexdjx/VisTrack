...
import ltr.models.target_classifier.features_add as clf_features
...
@model_constructor
def fudimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    ################################################################################
    # my add
    clf_feature_extractor = clf_features.AWFF(norm_scale=norm_scale)  # AWFF
    ################################################################################
    ...
    # DiMP network
    net = FuDiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                    classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net


@model_constructor
def fudimpnet_awff(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                classification_layer=None, feat_stride=16, backbone_pretrained=True,
                init_filter_norm=False, out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                score_act='relu', act_param=None, target_mask_act='sigmoid',
                detach_length=float('Inf'), frozen_backbone_layers=()):

    if classification_layer is None:
        classification_layer = ['layer2', 'layer3']
    ...
    # -- my modify-- ##############################################################
    clf_feature_extractor = clf_features.AWFF(norm_scale=norm_scale)
    ###############################################################################
