...
class DenseBoxRegressor(nn.Module):
    ...
    def forward(self, feat, filter):
        ...
        feats_tower = self.tower(
            feats_att.reshape(-1, self.num_channels, feat.shape[-2], feat.shape[-1]))  # (nf*ns, c, h, w)
...
class Prob_Head(nn.Module):
    ...
    def forward(self, train_feat, test_feat, train_prob, test_prob, train_bb, *args, **kwargs):
        ...
        # Extract features
        train_feat = self.extract_head_feat(train_feat, train_prob, num_sequences)
        test_feat = self.extract_head_feat(test_feat, test_prob, num_sequences)
    ...
    def extract_head_feat(self, feat, prob, num_sequences=None):
        """Extract classification features based on the input backbone features."""
        if self.feature_extractor is None:
            return feat
        if num_sequences is None:
...
class Prompt_Head(nn.Module):
    ...
    def extract_head_feat(self, feat, pro, num_sequences=None):
        """Extract classification features based on the input backbone features."""
        if self.feature_extractor is None:
            return feat
        ...
        output = self.feature_extractor[2](self.feature_extractor[0](feat) * self.feature_extractor[1](pro))  # up

        # output = self.feature_extractor(feat,pro)  # patch embedding
