...
from pytracking.features.preprocessing import sample_patch_multiscale, sample_patch, patch_prob_transformed
...
from pytracking.utils.box_utils import box_cxcywh_to_xyxy


class CAT(BaseTracker):  # color attention tracker
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.get('score_match', False):
            self.prroi_pool = PrRoIPool2D(1, 1, 1 / 16)  # my add

            pos_c = (self.img_sample_sz - 1) // 2
            target_sz = self.target_sz[[1, 0]] / self.target_scale
            target_bbox = torch.cat((pos_c, target_sz))
            target_bbox = box_cxcywh_to_xyxy(target_bbox)

            if self.params.get('cls_feat', False):
                self.template_feat = self.generate_target_feat(self.cls_feat, target_bbox)
            else:
                self.template_feat = self.generate_target_feat(init_backbone_feat['layer3'][0], target_bbox)

        # update or not
        if self.params.get('prob_use', True):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.params.get('score_match', False):
            ...
            target_bbox = self.get_iounet_box(self.pos, self.target_sz, sample_pos[scale_ind, :],
                                              sample_scales[scale_ind])

            target_bbox[2:] = target_bbox[:2] + target_bbox[2:]

            if self.params.get('cls_feat', False):
                new_target_feat = self.generate_target_feat(test_x[0], target_bbox)
            else:
                new_target_feat = self.generate_target_feat(backbone_feat['layer3'][0], target_bbox)

            # for visual crop target
            # target_bbox = target_bbox.int()
            # im_patches = im_patches[0]
            # im_patches = im_patches.permute(1, 2, 0)
            # im = im_patches[target_bbox[1]:target_bbox[3], target_bbox[0]:target_bbox[2], :]
            ...
            # cos_sim
            sim = torch.cosine_similarity(self.template_feat.view(1, -1), new_target_feat.view(1, -1))

            if flag == 'normal' and sim < 0.5:
                flag = 'uncertain'
        ...
        # my add ###############################################################
        if self.params.get('prob_use', True) and self.params.get('prob_update', False):
            if update_flag:
                self.update_lr = self.params.learning_rate
            else:
                self.update_lr = 0
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        if not self.params.get('init_prob', False):
            ...
            if self.params.get('prob_use', True):
                ...
                # --target probability-- ##############################################
                target_prob = get_target_probability(im_patches[0], self.target_sz / self.target_scale)

                if self.params.get('prob_mean', False):
                    target_prob = torch.mean(target_prob, dim=1, keepdim=True)
    ...
    # --score matching--##################################################################################
    def generate_target_feat(self, feat: torch.Tensor, target_bbox):
        # Add batch_index to rois x1y1x2y2
        batch_index = torch.arange(1, dtype=torch.float32).reshape(-1, 1)
        roi = torch.cat((batch_index, target_bbox.reshape(-1, 4)), dim=1)

        target_feat = self.prroi_pool(feat.unsqueeze(0), roi.to(feat.device))
    ...
    def get_classification_features(self, backbone_feat):
        with torch.no_grad():
            return self.net.extract_classification_feat(backbone_feat, self.target_prob.cuda())
    ...
    def generate_init_samples(self, im: torch.Tensor):
        ...
        if self.params.get('prob_use', True):
            # --target probability-- #########################################################################
            im_patch, _ = sample_patch(im, self.init_sample_pos, self.init_sample_scale * aug_expansion_sz,
                                       aug_expansion_sz)

            target_prob = get_target_probability(im_patch[0], self.target_sz / self.target_scale)

            im_patches, target_prob = patch_prob_transformed(im_patch, target_prob, self.transforms)

            if self.params.get('prob_mean', False):
                target_prob = torch.stack([torch.mean(i, dim=0, keepdim=True) for i in target_prob], dim=0)
