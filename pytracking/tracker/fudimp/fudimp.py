...
class FuDiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # out = {'target_bbox': output_state, 'score_index': [self.psr_score_index, self.apce_score_index,
        # self.psme_score_index]}
    ...
    def localize_advanced2(self, scores, sample_pos, sample_scales):
        ...
        if self.params.get('score_psme', False):
            ...
            if self.frame_num == 2:
                self.score_threshold = score_index * self.params.score_threshold
        ...
        if max_score2 > self.params.distractor_threshold * max_score1:
            ...
            # If also the distractor is close, return with the highest score
            return translation_vec1, scale_ind, scores_hn, 'uncertain'
        ...
        # --my add-- #######################
        if score_index < self.score_threshold:
            return translation_vec1, scale_ind, scores_hn, 'uncertain'
    ...
    def optimize_boxes_relative(self, iou_features, init_boxes):
        """Optimize iounet boxes with the relative parametrization is in PrDiMP"""
        output_boxes = init_boxes.view(1, -1, 4).to(self.params.device)
        step_length = self.params.box_refinement_step_length
        if isinstance(step_length, (tuple, list)):
