...
from ltr.models.tracking import fudimpnet
...
def run(settings):
    settings.description = 'Feature Fusion based on SuperDiMP, SuperDiMP: Combines the DiMP classifier with the PrDiMP ' \
                           'bounding box regressor and better' \
    ...
    net = fudimpnet.fudimpnet_awff_att(filter_size=settings.target_filter_sz, optim_init_step=0.9, optim_init_reg=0.1,
                                       init_gauss_sigma=output_sigma * settings.feature_sz, num_dist_bins=100,
                                       bin_displacement=0.1, mask_init_factor=3.0,
