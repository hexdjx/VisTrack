...
from .optim_atom import ConvProblem, FactorizedConvProblem
...
class ATOM(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        def _ms_ratio(scale_factors):
            ...
            sf = torch.from_numpy(np.array(multi_scales)).float()
            sample_scales = self.target_scale * sf
        ...
        if getattr(self.params, 'is_multiscale_var', False) is True:
                ...
                for i in range(iter):
                    if scale_ind == 0:
                        ...
                        if new_scale_ind == 1:
                            ...
                            return x0, translation_vec0, sample_scales[1], s0[1, ...], flag0
                        if i == iter - 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[0], s0[0, ...], flag0
                    else:
                        ...
                        if new_scale_ind == 0:
                            ...
                            return x1, translation_vec1, sample_scales[0], s1[0, ...], flag1
                        if i == iter - 1:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[1], s1[1, ...], flag1
            ...
            if scale_ind == 1:
                ...
                x, translation_vec, final_scale, final_s, flag = _choice_scale(scale_ind, self.params.scale_iter)
                ...
                for i in range(iter):
                    ...
                    if scale_ind <= 2:
                        ...
                        if new_scale_ind >= 2:
                            # if new_scale_ind == 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[new_scale_ind], s0[new_scale_ind, ...], flag0
                        if i == iter - 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[new_scale_ind], s0[new_scale_ind, ...], flag0
                    else:
                        ...
                        if new_scale_ind <= 1:
                            # if new_scale_ind == 0:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[new_scale_ind], s1[new_scale_ind, ...], flag1
                        if i == iter - 1:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[new_scale_ind], s1[new_scale_ind, ...], flag1
        ...
        if update_flag:
            ...
            if getattr(self.params, 'is_multiscale_ratio', False) is True:
                ...
                train_y = self.get_label_function(sample_pos, ss)

            if getattr(self.params, 'is_multiscale_var', False) is True:
                train_x = x
                train_y = self.get_label_function(sample_pos, final_scale)  # Create label for sample
