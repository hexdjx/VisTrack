...
class FactorizedConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, yf: TensorList, reg_filter: torch.Tensor,
                 init_proj_mat: TensorList, params, sample_weights: torch.Tensor = None):
        ...
        self.diag_M = (1 - self.params.precond_reg_param) * (self.params.precond_data_param * self.sample_energy +
                                                             (
                                                                         1 - self.params.precond_data_param) * self.sample_energy.mean(
                    1, keepdim=True)) + \
    ...
    def ip_output(self, a: TensorList, b: TensorList):
        ...
        ip_P_reg = sum(a_P_reg.reshape(-1) @ b_P_reg.reshape(-1))  # view-->reshape
...
class FilterOptim(optimization.ConjugateGradientBase):
    ...
    def run(self, num_iter, new_xf: TensorList = None):
        ...
        if new_xf is not None:
            ...
            if self.sample_energy is None:
                ...
                self.sample_energy = (
                                                 1 - self.params.precond_learning_rate) * self.sample_energy + self.params.precond_learning_rate * new_sample_energy
        ...
        self.diag_M = (1 - self.params.precond_reg_param) * (self.params.precond_data_param * self.sample_energy +
                                                             (
                                                                         1 - self.params.precond_data_param) * self.sample_energy.mean(
                    1, keepdim=True)) + self.params.precond_reg_param * self.reg_energy
    ...
    def A(self, hf: TensorList):
        ...
        for hfe, hfe_out, reg_filter in zip(hf, hf_out, self.reg_filter):
            ...
            hfe_out += hfe_conv.reshape(hfe.shape[0], hfe.shape[1], 2, hfe.shape[2], hfe.shape[3]).permute(0, 1, 3, 4,
                                                                                                           2)
