...
class AttVerifyToMPActor(BaseActor):
    """Actor for training the DiMP network."""
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        target_scores, bbox_preds, target_emb = self.net(train_imgs=data['train_images'],
                                                         test_imgs=data['test_images'],
                                                         train_prob=data['train_target_prob'],
                                                         test_prob=data['test_target_prob'],
                                                         train_target_bb=data['train_target_anno'],
                                                         test_target_bb=data['test_target_anno'],
                                                         train_bb=data['train_anno'],
                                                         train_label=data['train_label'],
                                                         train_ltrb_target=data['train_ltrb_target'])
        ...
        train_feat_embedding, test_feat_embedding = target_emb #n_seq, bs, n_bb, emb
        train_feat_embedding = train_feat_embedding.permute(0, 2, 1, 3)
        test_feat_embedding = test_feat_embedding.permute(0, 2, 1, 3)

        tri_loss = 0
        for train_feat_emb in train_feat_embedding:
            test_feat_emb = test_feat_embedding[0]
            for embedding in train_feat_emb[1:]:
                loss_tmp = self.objective['tri'](train_feat_emb[0], test_feat_emb[0], embedding)
                tri_loss += loss_tmp

            for embedding in test_feat_emb[1:]:
                loss_tmp = self.objective['tri'](test_feat_emb[0], train_feat_emb[0], embedding)
                tri_loss += loss_tmp

        loss = self.loss_weight['giou'] * loss_giou + self.loss_weight['test_clf'] * clf_loss_test + \
               self.loss_weight['tri'] * tri_loss
        ...
        stats = {'Loss/total': loss.item(),
                 ...
                 'Loss/tri': tri_loss.item(),
                 'Loss/weighted_tri': self.loss_weight['tri'] * tri_loss.item(),
