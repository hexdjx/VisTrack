...
from ltr.models.tracking import promptdimpnet
...
def run(settings):
    ...
    data_processing_train = processing.PromptKLDiMPProcessing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.PromptKLDiMPProcessing(search_area_factor=settings.search_area_factor,
                                                            output_sz=settings.output_sz,
                                                            center_jitter_factor=settings.center_jitter_factor,
                                                            scale_jitter_factor=settings.scale_jitter_factor,
                                                            crop_type='inside_major',
    ...
    net = promptdimpnet.prodimpnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
                                     out_feature_dim=512, optim_init_step=0.9, optim_init_reg=0.1,
                                     init_gauss_sigma=output_sigma * settings.feature_sz, num_dist_bins=100,
                                     bin_displacement=0.1, mask_init_factor=3.0, target_mask_act='sigmoid',
                                     score_act='relu',
                                     frozen_backbone_layers=['conv1', 'bn1', 'layer1', 'layer2'], mlp_dim=mlp_dim)
    ...
    actor = tracking_actors.PromptDiMPActor(net=net, objective=objective, loss_weight=loss_weight, prompt_mode=prompt_mode)
