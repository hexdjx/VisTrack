...
class TransTSampler(torch.utils.data.Dataset):
    """ See TrackingSampler."""
...
# --Scale Estimator--###############################################################
class SESampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        args:
            ...
            processing - An instance of Processing class which performs the necessary processing of the data.
            frame_sample_mode - Either 'internal' or 'causal'. If 'causal', then the test frames are sampled in a causal
                                manner.
        """
        self.datasets = datasets
    ...
    def __getitem__(self, index):
        ...
        train_frames, train_anno, _ = dataset.get_frames(seq_id, train_frame_ids, seq_info_dict)

        test_frames, test_anno, _ = dataset.get_frames(seq_id, test_frame_ids, seq_info_dict)

        if 'mask' in train_anno:
            # Prepare data
            data = TensorDict({'train_images': train_frames,
                               'train_masks': train_anno['mask'],
                               'train_anno': train_anno['bbox'],
                               'test_images': test_frames,
                               'test_masks': test_anno['mask'],
                               'test_anno': test_anno['bbox'],
                               'dataset': dataset.get_name(),
                               'mask': True})
        else:
            # generate coarse mask form bbo
            H, W, _ = train_frames[0].shape

            data = TensorDict({'train_images': train_frames,
                               'train_masks': self.bbox2mask([H, W], train_anno['bbox']),
                               'train_anno': train_anno['bbox'],
                               'test_images': test_frames,
                               'test_masks': self.bbox2mask([H, W], test_anno['bbox']),
                               'test_anno': test_anno['bbox'],
                               'dataset': dataset.get_name(),
                               'mask': False})
