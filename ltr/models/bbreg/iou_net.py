...
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=False):
...
def cross_attention(feat1, feat2):
    b, c, w, h = feat1.size()
    q = k = feat1.reshape(b, c, w * h)
    v = feat2.reshape(b, c, w * h)
...
class IoUNet(nn.Module):
    ...
    def __init__(self, input_dim=(128, 256), pred_input_dim=(256, 256), pred_inter_dim=(256, 256)):
        super().__init__()
        # _r for reference
        self.conv3_1r = conv(input_dim[0], 256, kernel_size=1, stride=1, padding=0)
        ...
        # _t for test
        self.conv3_1t = conv(input_dim[0], 256, kernel_size=1, stride=1, padding=0)
    ...
    def get_modulation(self, feat, bb):
        ...
        c3_4_r = F.interpolate(c3_r, scale_factor=0.5, mode='bilinear')  # F.max_pool2d(c3_r, 3, stride=2, padding=1)

        c4_r = self.conv4_1r(feat4_r)
        c4_3_r = F.interpolate(c4_r, scale_factor=2, mode='bilinear')

        c3_att_r = cross_attention(c4_3_r, c3_r)
        c4_att_r = cross_attention(c3_4_r, c4_r)

        roi3r = self.prroi_pool3r(c3_att_r, roi1)
        roi4r = self.prroi_pool4r(c4_att_r, roi1)
        pool3_r = self.avg_pool(roi3r)

        fc34_r = torch.cat((pool3_r, roi4r), dim=1)

        att_3_r = self.fc3_r(fc34_r)
        att_4_r = self.fc4_r(fc34_r)
    ...
    def get_iou_feat(self, feat2):
        ...
        c3_4_t = F.interpolate(c3_t, scale_factor=0.5, mode='bilinear')  # F.max_pool2d(c3_r, 3, stride=2, padding=1)

        c4_t = self.conv4_1t(feat4_t)
        c4_3_t = F.interpolate(c4_t, scale_factor=2, mode='bilinear')

        c3_att_t = cross_attention(c4_3_t, c3_t)
        c4_att_t = cross_attention(c3_4_t, c4_t)

        return c3_att_t, c4_att_t
