...
from pytracking.utils.plotting import show_tensor, plot_graph, mesh_score
...
class EnDiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        ###########################################################
        # my add
        verifier_flag = self.params.get('verifier_flag', False)
        if verifier_flag:
            # use classification feature
            self.init_target_feat = self.generate_target_feat(im)
        ############################################################
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        verifier_flag = self.params.get('verifier_flag', False)
        if verifier_flag:
            new_target_feat = self.generate_target_feat(im)
            sim = torch.cosine_similarity(self.init_target_feat, new_target_feat)
            # print(sim)
            if self.params.adaptive_flag:
                # adaptive set threshold
                if self.frame_num == 2:
                    normal_threshold = sim.item() * self.params.beta
                    hard_neg_threshold = sim.item() * self.params.beta ** 2
                    self.params.normal_threshold = normal_threshold
                    self.params.hard_neg_threshold = hard_neg_threshold

            if sim > self.params.normal_threshold:
                flag = 'normal'
            elif sim > self.params.hard_neg_threshold:
                flag = 'hard_negative'
            else:
                flag = 'uncertain'
        ##############################################################
        ...
        if self.visdom is not None:
            ...
            show_tensor(score_map, self.frame_num, 5, title='Max score = {:.2f}'.format(max_score))
    ...
    def generate_target_feat(self, im: torch.Tensor):
        ...
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone(target_patch)
            target_feat = self.get_classification_features(backbone_feat)
            target_feat = nn.AdaptiveAvgPool2d((1, 1))(target_feat)
            target_feat = target_feat.view(target_feat.shape[0], -1)
        return target_feat
