...
from .optim_eco import FilterOptim, FactorizedConvProblem
...
class ECO(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # my multiscale ratio method
        def _ms_ratio(scale_factors):
            multi_scales = []
            for s1 in scale_factors:
                for s2 in scale_factors:
                    multi_scales.append([s1, s2])
            ms = torch.from_numpy(np.array(multi_scales)).float()
            sample_scales = self.target_scale * ms
            return sample_scales, ms
        ...
        translation_vec, scale_ind, s = self.localize_target(sf, ms)
        if getattr(self.params, 'is_multiscale_var', False) is True:
            def _choice_scale(scale_ind, iter=2):
                base_scale_factors = self.params.scale_factors
                for i in range(iter):
                    if scale_ind == 0:
                        base_scale_factors = base_scale_factors[0] * base_scale_factors
                        if i == 0:
                            scale_factors = base_scale_factors[0:2]
                        else:
                            scale_factors = base_scale_factors[1:3]
                        sample_scales = self.target_scale * scale_factors
                        test_x = self.extract_processed_sample(im, self.pos, sample_scales, self.img_sample_sz)
                        scores_raw = self.apply_filter(test_x)
                        translation_vec0, new_scale_ind, s0 = self.localize_target(scores_raw)
                        if new_scale_ind == 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[1], s0[1, ...]
                        if i == iter - 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[0], s0[0, ...]
                    else:
                        base_scale_factors = base_scale_factors[-1] * base_scale_factors
                        if i == 0:
                            scale_factors = base_scale_factors[1:3]
                        else:
                            scale_factors = base_scale_factors[0:2]
                        ...
                        test_x = self.extract_processed_sample(im, self.pos, sample_scales, self.img_sample_sz)
                        scores_raw = self.apply_filter(test_x)
                        translation_vec1, new_scale_ind, s1 = self.localize_target(scores_raw)
                        if new_scale_ind == 0:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[0], s1[0, ...]
                        if i == iter - 1:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[1], s1[1, ...]

            if scale_ind == 1:
                final_scale = sample_scales[scale_ind]
                final_s = s[1, ...]
                x = TensorList([x[scale_ind:scale_ind + 1, ...] for x in test_x])
            else:
                x, translation_vec, final_scale, final_s = _choice_scale(scale_ind, self.params.scale_iter)
                ...
                for i in range(iter):
                    ...
                    if scale_ind <= 2:
                        ...
                        if new_scale_ind >= 2:
                            # if new_scale_ind == 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[new_scale_ind], s0[new_scale_ind, ...]
                        if i == iter - 1:
                            x0 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x0, translation_vec0, sample_scales[new_scale_ind], s0[new_scale_ind, ...]
                    else:
                        ...
                        if new_scale_ind <= 1:
                            # if new_scale_ind == 0:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[new_scale_ind], s1[new_scale_ind, ...]
                        if i == iter - 1:
                            x1 = TensorList([x[new_scale_ind:new_scale_ind + 1, ...] for x in test_x])
                            return x1, translation_vec1, sample_scales[new_scale_ind], s1[new_scale_ind, ...]

            # if scale_ind == 1:
            if scale_ind >= 3 and scale_ind <= 5:
                final_scale = sample_scales[scale_ind]
                # final_scale = torch.from_numpy(np.array([final_scale, final_scale])).float()
                final_s = s[scale_ind, ...]
                x = TensorList([x[scale_ind:scale_ind + 1, ...] for x in test_x])
            else:
                x, translation_vec, final_scale, final_s = _choice_scale(scale_ind, self.params.scale_iter)
        ...
        if getattr(self.params, 'is_multiscale_ratio', False) is True:
            ss = sample_scales[scale_ind].squeeze(0)
            self.update_state(sample_pos + translation_vec, ss)
        if getattr(self.params, 'is_multiscale_var', False) is True:
            self.update_state(sample_pos + translation_vec, final_scale)
        if getattr(self.params, 'is_multiscale_var_ratio', False) is True:
            ss = final_scale.squeeze(0)
            # self.update_state(sample_pos + translation_vec, final_scale)
            self.update_state(sample_pos + translation_vec, ss)
            # self.target_scale = torch.mean(self.target_scale)
        ...
        # Get train sample
        if getattr(self.params, 'is_multiscale_ratio', False) is True:
            train_xf = TensorList([x[scale_ind:scale_ind + 1, ...] for x in test_x])
        else:
            train_xf = x
        ######################################################################

        # Shift the sample
        shift_samp = 2 * math.pi * (self.pos - sample_pos) / (sample_scales[scale_ind].squeeze(0) * self.img_support_sz)
        train_xf = fourier.shift_fs(train_xf, shift=shift_samp)
    ...
    def localize_target(self, sf: TensorList, ms=None):
        ...
        if self.params.score_fusion_strategy in ['sum', 'weightedsum']:
            if getattr(self.params, 'is_multiscale_var', False) is True:
                ...
                translation_vec *= ms[scale_ind, :].squeeze(0)
