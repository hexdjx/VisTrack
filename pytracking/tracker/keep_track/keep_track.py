...
from pytracking.utils.score_index import *
...
class KeepTrack(BaseTracker):
    ...
    def localize_target_by_candidate_matching(self, im_patches1, backbone_feat1, score_map1, search_area_box1,
                                              sample_pos, sample_scales, img_shape):
    ...
    def extract_descriptors_and_keypoints(self, backbone_feat, score_map, search_area_box):
        ...
        with torch.no_grad():
            descriptors = self.target_candidate_matching_net.descriptor_extractor.get_descriptors(frame_feat_clf,
                                                                                                  tsm_coords)
    ...
    # additional add relative to dimp
    def init_target_labels(self, train_x: TensorList):
        ...
        self.sigma = (self.feature_sz / self.img_support_sz * self.base_target_sz).prod().sqrt() * \
                     output_sigma_factor * torch.ones(2)
    ...
    def update_memory(self, sample_x: TensorList, sample_y: TensorList, target_box, learning_rate=None,
        ...
        if (self.candidate_collection is None or self.candidate_collection.object_id_of_selected_candidate == 0):
            target_label_certainty = torch.max(target_label_certainty,
                                               torch.sqrt(target_label_certainty.clone().detach()))
    ...
    def update_sample_weights_based_on_certainty(self, certainties, sample_weights, previous_replace_ind,
                                                 num_stored_samples, num_init_samples, learning_rate=None):
        # Update weights and get index to replace
        replace_ind = []
        for cert, sw, prev_ind, num_samp, num_init in zip(certainties, sample_weights, previous_replace_ind,
    ...
    def visualize_candidate_matching(self, data):
        ...
        def add_matching_probability(axis, prob, id0, id1, num_entry, offset, color):
            x = offset + 10
            y = num_entry * 20
            axis.text(x, y, '{}--[{:0.1f}]--{}'.format(id0, 100 * prob, id1), ha="left", va="center", color=color,
                      fontsize=10)
