...
import ltr.models.bbreg.SEnet as SEx
import torch
...
from ltr.dataset import Got10k, Lasot, MSCOCOSeq, ImagenetVID, ImagenetDET, Saliency, YouTubeVOS
...
def run(settings):
    ...
    data_processing_train = processing.SEMaskProcessing(search_area_factor=settings.search_area_factor,
    ...
    got_10k_train = Got10k(split='train')
    ...
    # - mask datasets
    youtube_vos = YouTubeVOS()
    saliency = Saliency()

    # The sampler for training
    '''Build training dataset. focus on "__getitem__" and "__len__"'''
    dataset_train = sampler.SEMaskSampler([lasot_train,got_10k_train,coco_train,imagenet_vid,imagenet_det,youtube_vos,saliency],
                                            [1, 1, 1, 1, 1, 2, 3],
    ...
    data_processing_val = processing.SEMaskProcessing(search_area_factor=settings.search_area_factor,
                                                        output_sz=settings.output_sz,
                                                        center_jitter_factor=settings.center_jitter_factor,
                                                        scale_jitter_factor=settings.scale_jitter_factor,
                                                        mode='sequence',
    ...
    dataset_val = sampler.SEMaskSampler([lasot_test], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,
    ...
    # Create network
    net = SEx.SE_resnet34(backbone_pretrained=True,
    ...
    # Set objective
    objective = {'corner': nn.MSELoss(), 'mask': nn.BCELoss()}

    # Create actor, which wraps network and objective
    actor = actors.CornerMaskActor(net=net, objective=objective)
