...
from pytracking.utils.box_utils import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh
...
class CAToMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        with torch.no_grad():
            if self.params.get('prob_use', True):
                ...
                test_x = self.net.head.extract_head_feat(test_x)
        # ---------------------------------------------------------
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        if self.params.get('prob_use', True):
            if not self.params.get('init_prob', False):
                ...
                # target_prob = get_target_probability(im_patches[0], self.target_sz / sample_scales[0])
    ...
    def get_target_prob(self, img, bbox):
        ...
        return P.permute(2, 0, 1).to(self.params.device)

    def get_target_probability(self, img, bbox, inner_padding=0.2):
        def calHist_mask(im, mask, n_bins=16):
            h, w, c = im.shape
        ...
        bbox[:2] = bbox[:2] + (bbox[2:] - 1) / 2

        avg_dim = 0.5 * bbox[2:].sum()
        bbox[2:] = bbox[2:] - avg_dim * inner_padding
        ...
        # img_crop =img[round(y):round(y + bh), round(x):round(x + bw)]

        import matplotlib.pyplot as plt
        # plt.figure(10)
        # plt.imshow(img_crop.int())
        # plt.show()
        ...
        return P.permute(2, 0, 1).unsqueeze(0).to(self.params.device)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        with torch.no_grad():
            if self.params.get('prob_use', True):
                ...
                x = self.net.head.extract_head_feat(x)
