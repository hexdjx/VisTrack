...
@model_constructor
def fudimpnet_awff_att(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    ################################################################################
    # my add
    clf_feature_extractor = clf_features.AWFFatt(norm_scale=norm_scale)
    ################################################################################
...
@model_constructor
def fudimpnet_ff(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                 classification_layer='layer3', feat_stride=16, backbone_pretrained=True, init_filter_norm=False,
                 out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                 mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                 score_act='relu', act_param=None, target_mask_act='sigmoid',
    ...
    ################################################################################
    # my add
    clf_feature_extractor = clf_features.FetureFusion(norm_scale=norm_scale)
    ################################################################################
