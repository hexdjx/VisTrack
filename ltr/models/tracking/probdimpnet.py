...
class ProbDiMPnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_prob, test_prob, train_bb, test_proposals, *args, **kwargs):
        ...
        # train_prob = train_prob.reshape(-1, *train_prob.shape[-3:])
        # test_prob = test_prob.reshape(-1, *test_prob.shape[-3:])

        train_prob = F.upsample(train_prob.reshape(-1, *train_prob.shape[-3:]), size=(22, 22), mode='bilinear')
        test_prob = F.upsample(test_prob.reshape(-1, *test_prob.shape[-3:]), size=(22, 22), mode='bilinear')

        # Run classifier module
        target_scores = self.classifier(train_feat_clf, test_feat_clf, train_prob, test_prob, train_bb, *args, **kwargs)
...
@model_constructor
def probdimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                  ...
                  detach_length=float('Inf'), frozen_backbone_layers=(), prob_mean=False):
    # Backbone
    ...
    classifier = target_clf.ProbLinearFilter(filter_size=filter_size, filter_initializer=initializer,
                                             filter_optimizer=optimizer, feature_dim=feature_dim, out_dim=out_feature_dim,
                                             norm_scale=norm_scale, prob_mean=prob_mean)
    ...
    # DiMP network
    net = ProbDiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                      classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net

# for test w/o mlp
@model_constructor
def prob1dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                  classification_layer='layer3', feat_stride=16, backbone_pretrained=True, init_filter_norm=False,
                  out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                  mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                  score_act='relu', act_param=None, target_mask_act='sigmoid',
