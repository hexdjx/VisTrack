...
class IoUNet(nn.Module):
    """Network module for IoU prediction. It uses two backbone feature layers as input.
    args:
    ...
    def __init__(self, input_dim=(128, 256), pred_input_dim=(256, 256), pred_inter_dim=(256, 256)):
        super().__init__()
        # _r for reference
        self.conv3_1r = conv(input_dim[0], 128, kernel_size=1, stride=1, padding=0)
        self.conv4_1r = conv(input_dim[1], 256, kernel_size=1, stride=1, padding=0)
        self.conv3_2r = conv(128 + 256, 256, kernel_size=3, stride=1)
        self.conv4_2r = conv(128 + 256, 256, kernel_size=3, stride=1)
        self.prroi_pool3r = PrRoIPool2D(3, 3, 1 / 8)
        self.prroi_pool4r = PrRoIPool2D(1, 1, 1 / 16)
        # self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.conv3_3r = conv(256, pred_input_dim[0], kernel_size=3, stride=1, padding=0)
        self.fc3_r = conv(256, pred_input_dim[0], kernel_size=1, stride=1, padding=0)
        self.fc4_r = conv(256, pred_input_dim[1], kernel_size=1, stride=1, padding=0)

        # _t for test
        self.conv3_1t = conv(input_dim[0], 128, kernel_size=1, stride=1, padding=0)
        self.conv4_1t = conv(input_dim[1], 256, kernel_size=1, stride=1, padding=0)
        self.conv3_2t = conv(128 + 256, 256, kernel_size=3, stride=1)
        self.conv4_2t = conv(128 + 256, 256, kernel_size=3, stride=1)
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        modulation = [f.reshape(1, num_sequences, -1).repeat(num_images, 1, 1).reshape(num_sequences * num_images, -1)
                      for f in modulation]
    ...
    def predict_iou(self, modulation, feat, proposals):
        ...
        att_3_r, att_4_r = modulation
        c3_t, c4_t = feat

        batch_size = c3_t.size()[0]

        # Modulation
        c3_t_att = c3_t * att_3_r.reshape(batch_size, -1, 1, 1)
        c4_t_att = c4_t * att_4_r.reshape(batch_size, -1, 1, 1)
    ...
    def get_modulation(self, feat, bb):
        ...
        c3_4_r = F.max_pool2d(c3_r, 3, stride=2, padding=1)

        c4_r = self.conv4_1r(feat4_r)
        # c4_3_r = F.interpolate(c4_r, scale_factor=0.5, mode='bilinear')
        c4_3_r = F.interpolate(c4_r, scale_factor=2, mode='bilinear')

        c34_r = torch.cat([c3_r, c4_3_r], dim=1)
        c43_r = torch.cat([c4_r, c3_4_r], dim=1)

        c3_2_r = self.conv3_2r(c34_r)
        c4_2_r = self.conv4_2r(c43_r)

        roi3r = self.prroi_pool3r(c3_2_r, roi1)
        roi4r = self.prroi_pool4r(c4_2_r, roi1)
        # pool3_r = self.avg_pool(roi3r)
        att_3_r = self.conv3_3r(self.fc3_r(roi3r))
        att_4_r = self.fc4_r(roi4r)

        return att_3_r, att_4_r
    ...
    def get_iou_feat(self, feat2):
        ...
        c3_4_t = F.max_pool2d(c3_t, 3, stride=2, padding=1)

        c4_t = self.conv4_1t(feat4_t)
        c4_3_t = F.interpolate(c4_t, scale_factor=2, mode='bilinear')

        c34_t = torch.cat([c3_t, c4_3_t], dim=1)
        c43_t = torch.cat([c4_t, c3_4_t], dim=1)

        c3_2_t = self.conv3_2r(c34_t)
        c4_2_t = self.conv4_2r(c43_t)

        return c3_2_t, c4_2_t
